...
/*
   Class: SimpleMSController
   Main class that controls the Multiple Select activity type. This class
   manages the evaluator and display classes for the activity. 
*/
class enspire.interactivity.control.SimpleMSController 
	implements IActivity {
		
	private var evaluator:BasicChoiceEvaluator;
	private var activityArgs:Object;
	private var activityProfile:Object;
	private var activityDisplay:SimpleMSDisplay;
	
	
	public function SimpleMSController() {
	...
	public function resume():Void {
		activityDisplay.startFeedbackPhase();
	}
	...
	private function setupEvaluator() {
		...
		while(activityArgs["sChoice"+i] != undefined) {
			...
			evaluator.setMultipleSelect(true);
			i++;
		}
	...
	public function setupActivityDisplay(clip:MovieClip):Void {
		activityDisplay = new SimpleMSDisplay();
		var mcActivity = Interactivity.iTimeline.attachMovie(activityProfile["sSkin"],"masdg", Interactivity.iTimeline.getNextHighestDepth());
	...
	public function evaluateActivity():IEvaluation {
		...
		if(activityProfile["sFeedbackType"] == "specific") {
			submitBtn._visible = false;
		}
