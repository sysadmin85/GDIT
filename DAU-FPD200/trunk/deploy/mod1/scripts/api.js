<!--
// FS SCORM adapted for Enspire Learning Courseware
// derivative 1.0		12/20/2004
...
function flashSco_DoFSCommand(command, args){
	var flashScoObj = g_bIsInternetExplorer ? flashSco : document.flashSco;
	...
	if (cmd.substring(0,3) == "LMS"){
		...
		if ( cmd == "LMSInitialize" ){
			...
			if ( cmd == "LMSGetValue") {
				flashScoObj.SetVariable(arg2, SCOGetValue(arg1));
			}	else if ( cmd == "LMSGetLastError") {
				flashScoObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetErrorString") {
				flashScoObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetDiagnostic") {
				flashScoObj.SetVariable(arg2, SCOGetDiagnostic(arg1));
			}	else {
				// for unknown LMSGetxxxx extension
				v = eval('g_objAPI.' + cmd + '(\"' + arg1 + '\")');
				flashScoObj.SetVariable(arg2,v);
			}
